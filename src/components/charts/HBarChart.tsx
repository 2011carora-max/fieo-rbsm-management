...
export function HBarChart({ data, height = 260, color = '#1f4a8a', valueFormat }: { data: SeriesPoint[]; height?: number; color?: string; valueFormat?: (v: number) => string }) {
  const uid = useId().replace(/:/g, '');
  const { tip, show, hide } = useChartTooltip();
  const width = 600;
  const pad = { top: 8, right: 48, bottom: 8, left: 110 };
  const innerW = width - pad.left - pad.right;
  const rowH = data.length ? Math.min(28, (height - pad.top - pad.bottom) / data.length) : 0;
  const max = Math.max(1, ...data.map((d) => d.value));

  return (
    <div className="relative w-full">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Horizontal bar chart">
        <defs>
